...
const ManageAdmin = () => {
  ...
  const activateAdmin = async (email) => {
    try {
      const res = await axios({
        url: import.meta.env.VITE_BASE_URL + '/SA/giveLoginAccess',
        method: 'post',
        data: { emailOfHA: email },
        headers: {
          "Content-Type": "application/json",
        },
        withCredentials: true,
      ...
      getAdmins(); // refresh table
    } catch (error) {
      console.log(error);
      toast.error("Error while activating admin", {
        position: "top-center",
      });
    }
  };
  ...
  const [colDefs, setColDefs] = useState([
    ...
    {
      field: "delete",
      headerClass: "font-bold border p-2 font-bold text-lg",
      cellRenderer: (e) => {
        // If admin is inactive, show Activate button
        if (!e.data.active) {
          return (
            <div className="flex w-full justify-center">
              <Button
                onClick={() => activateAdmin(e.data.email)}
                className="bg-green-700 h-8 px-3 hover:bg-green-500 text-white"
              >
                Activate
              </Button>
            </div>
        ...
        // If admin is active, show Delete (Deactivate) button
        return (
          <div className='flex w-full justify-center'>
            <Button
              onClick={() => deleteAdmin(e)}
              className='bg-red-700 h-8 p-[10px] hover:bg-red-500'
            >
              <ImBin size={10} />
            </Button>
          </div>
    ...
    <div className="relative min-h-screen w-full flex flex-col justify-start py-10 items-center">
     ...
     <div className='flex flex-col items-center justify-center w-full'>
      ...
      <div className='mt-4 mb-2 p-1 w-full lg:w-2/3 h-[380px] rounded-xl shadow-[0_3px_10px_rgb(0,0,0,0.2)]'>
        ...
        rowData={rowData}
        columnDefs={colDefs}
        rowClassRules={{'inactive-row': (params) => params.data && params.data.active === false}}/>
    ...
    <div className={`flex justify-center items-center -translate-y-full ${confirmModal ? 'translate-y-0' : ''} top-0 left-0 transition-all fixed w-full min-h-screen`}>
      ...
      <div className='min-w-[300px] z-50'>
        <Card>
          ...
          <CardFooter className='flex justify-between'>
            <Button ref={noRef} onClick={() => { setConfirmModal(false) }} className="bg-blue-500">Cancel</Button>
            <Button ref={yesRef} className="bg-red-700">Delete</Button>
          </CardFooter>
        </Card>
      </div>
    </div>
